...
const getUploadedFile = (files) => {
  const upload = files.resume || files.file;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const aiText = await generateAIText(buildPrompt(resumeText), {
      temperature: 0.1,
    ...
    console.error("Resume upload profile extraction error:", err);
    if (err?.code === 1009 || err?.message?.toLowerCase().includes("maxfilesize")) {
      return res.status(413).json({ error: "Resume PDF is too large. Please upload a file under 25 MB." });
    }
